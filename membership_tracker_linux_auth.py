...
class BusinessApp:
    def __init__(self, root, db_credentials=None):     
        # Setup Database with provided credentials
        print("Program initializing...")      
        if db_credentials:
            self.db = Database(
                dbname="",  # Specify your database name here
                user=db_credentials['username'],  # Pass the username from login
                password=db_credentials['password']  # Pass the password from login
            )
...
class LoginWindow:
    def __init__(self, root, app):
        ...
        self.label_user = tk.Label(self.frm, text="Username:")
        self.label_user.grid(row=0, column=0)
        
        self.entry_user = tk.Entry(self.frm)
        self.entry_user.grid(row=0, column=1)
        
        self.label_pass = tk.Label(self.frm, text="Password:")
        self.label_pass.grid(row=1, column=0)
        
        self.entry_pass = tk.Entry(self.frm, show="*")
        self.entry_pass.grid(row=1, column=1)
        
        self.login_button = tk.Button(self.frm, text="Login", command=self.verify_credentials)
    ...
    def verify_credentials(self):
        ...
        # Check credentials
        if self.check_linux_credentials(username, password):
            self.root.destroy()  # Close the login window
            
            # Create new root for Business App
            add_mem_root = tk.Tk()

            # Pass credentials to the app to open the database connection
            db_credentials = {
                'username': username,
                'password': password
            }
            self.app = BusinessApp(add_mem_root, db_credentials=db_credentials)
            self.app.__init__  # Start the business application
        else:
            messagebox.showerror("Login Failed", "Invalid username or password")

    def check_linux_credentials(self, username, password):
        try:
            # Use `sudo` to check if the credentials are valid
            result = os.system(f"echo {password} | sudo -S -u {username} whoami 2>/dev/null")
            return result == 0  # If the command succeeds, the credentials are valid
        except Exception as e:
...
if __name__ == "__main__":
    root = tk.Tk()
    login_window = LoginWindow(root, app=None)
    root.mainloop()
